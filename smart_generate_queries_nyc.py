...
# Constants
MAX_TEMPORAL_ZOOM_LEVEL = config.NYC.max_pickup_datetime_zoom
MAX_NUMERICAL_ZOOM_LEVEL = config.NYC.max_trip_distance_zoom
MAX_SPATIAL_ZOOM_LEVEL = config.NYC.max_pickup_coordinates_zoom
...
if __name__ == "__main__":
    ...
    number_of_trips = config.NYC.table_size
    min_pickup_datetime_dt = PostgreSQL.string_to_datetime(config.NYC.min_pickup_datetime)
    max_pickup_datetime_dt = PostgreSQL.string_to_datetime(config.NYC.max_pickup_datetime)
    max_number_of_seconds = (max_pickup_datetime_dt - min_pickup_datetime_dt).total_seconds()
    max_trip_distance_range = config.NYC.max_trip_distance - config.NYC.min_trip_distance
    max_width = config.NYC.max_pickup_coordinates_lng - config.NYC.min_pickup_coordinates_lng
    max_height = config.NYC.max_pickup_coordinates_lat - config.NYC.min_pickup_coordinates_lat
    ...
    for trip in sample_trips:
        ...
        trip_distance_range_start = max(trip_distance_range_center - trip_distance_range / 2, config.NYC.min_trip_distance)
        trip_distance_range_end = min(trip_distance_range_center + trip_distance_range / 2, config.NYC.max_trip_distance)
