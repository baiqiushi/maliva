...
# Evaluate DQN
#
# @param - dqn_model: DQN object, DQN model generated by smart_train_dqn_4d.py
...
#          * the outside list is ordered by the sample sizes ascending (i.e., 5k, 50k, 500k)
...
# @param - sample_pointer: int, [0~2], pointer to the sample size to use for the query_estimator. Default: 0 (5K)
#
# @return - (list of evaluated query objects, win_rate), each query object being
#           {id, keyword, planning_time, querying_time, total_time, win(1/0), plans_tried(x_x_x_x), reason}
def evaluate_dqn(dqn_model,
                 labeled_queries,
                 unit_cost,
                 time_budget,
...
if __name__ == "__main__":
    ...
    # evaluated DQN
    (evaluated_queries, win_rate) = evaluate_dqn(dqn_model,
                                                 labeled_queries,
                                                 unit_cost,
                                                 time_budget,
    ...
    if debug_qid == -1:
        ...
        print("-----------------------------------")
        print("qid,    keyword,    planning_time,    querying_time,    total_time,    win,    plans_tried,    reason")
        for query in evaluated_queries:
            print(str(query["id"]) + ",    " + "NULL" + ",    " + str(query["planning_time"]) + ",    " +
        ...
        with open(evaluated_queries_file, "w") as csv_out:
            csv_writer = csv.writer(csv_out, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
            for query in evaluated_queries:
                csv_writer.writerow([query["id"],
                                     "NULL",
                                     query["planning_time"],
                                     query["querying_time"],
                                     query["total_time"],
