...
import time
import torch
from smart_train_dqn_nyc import train_dqn
from smart_evaluate_dqn_nyc import evaluate_dqn
from smart_query_estimator_3d import Query_Estimator
from smart_util import Util


###########################################################
#  smart_select_dqn_nyc.py
#
# Purpose:
...
if __name__ == "__main__":
    ...
    train_queries = Util.load_labeled_queries_file_nyc(train_queries_file)

    # load validate queries into memory
    validate_queries = Util.load_labeled_queries_file_nyc(validate_queries_file)
    ...
    if version == '1' or version == '2':
        ...
        train_samples_labeled_sel_queries = Util.load_labeled_sel_queries_files_nyc(train_list_labeled_sel_file)
        validate_samples_labeled_sel_queries = Util.load_labeled_sel_queries_files_nyc(validate_list_labeled_sel_file)
        train_samples_query_sels = Util.load_sel_queries_files_nyc(train_list_sel_query_file)
        validate_samples_query_sels = Util.load_sel_queries_files_nyc(validate_list_sel_query_file)
    ...
    for i in range(1, number_of_tries + 1):
        ...
        (trained_dqn, fit_rate) = train_dqn(training_queries,
                                            unit_cost,
                                            time_budget,
                                            number_of_runs,
        ...
        (evaluated_validate_queries, eval_rate) = evaluate_dqn(trained_dqn,
                                                               validate_queries,
                                                               unit_cost,
                                                               time_budget,
